...
function install(systemId, pluginFullName) {
    const localSystem = utils.systemsById[systemId];
    if (utils.downloadURL != null) {
        if (pluginFullName == null) {
            if (localSystem != null)
                utils.emitError(`System ${systemId} is already installed.`);
...
function installSystem(systemId, downloadURL) {
    ...
    utils.downloadRelease(downloadURL, systemPath, (err) => {
        if (err != null)
            utils.emitError("Failed to install the system.", err);
        console.log("System successfully installed.");
        process.exit(0);
    });
}
function installPlugin(systemId, pluginFullName, downloadURL) {
    ...
    utils.downloadRelease(downloadURL, pluginPath, (err) => {
        if (err != null)
            utils.emitError("Failed to install the plugin.", err);
