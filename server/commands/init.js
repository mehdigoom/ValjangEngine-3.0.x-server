...
function init(systemId, pluginFullName) {
    ...
    if (systemFolderName == null)
        systemFolderName = systemId;
    let systemStats;
    try {
        systemStats = fs.lstatSync(`${utils.systemsPath}/${systemFolderName}`);
    }
    catch (err) { /* Ignore */ }
    if (pluginFullName == null) {
        if (systemStats != null)
            utils.emitError(`systems/${systemFolderName} already exists.`);
    }
    else {
        if (systemStats == null)
            utils.emitError(`systems/${systemFolderName} doesn't exist.`);
        let pluginStats;
        try {
            pluginStats = fs.lstatSync(`${utils.systemsPath}/${systemFolderName}/plugins/${pluginFullName}`);
        }
        catch (err) { /* Ignore */ }
        if (pluginStats != null)
            utils.emitError(`systems/${systemFolderName}/plugins/${pluginFullName} already exists.`);
    }
    utils.getRegistry((err, registry) => {
        ...
        if (pluginFullName == null)
            ...
            initPlugin(systemFolderName, systemId, pluginFullName);
    });
}
...
function initPlugin(systemFolderName, systemId, pluginName) {
