...
function update(systemId, pluginFullName) {
    // Update core
    if (systemId === "core" && pluginFullName == null) {
        let isDevFolder = true;
        try {
            fs.readdirSync(path.resolve(`${__dirname}/../../.git`));
        }
        catch (err) {
            isDevFolder = false;
        }
        if (isDevFolder)
            utils.emitError(`Core is a development version.`);
    ...
    if (pluginFullName == null) {
        ...
        utils.getRegistry((err, registry) => {
            if (err)
                utils.emitError("Error while fetching registry:", err.stack);
        ...
        utils.getRegistry((err, registry) => {
            if (err != null)
                utils.emitError("Error while fetching registry:");
            const registrySystem = registry.systems[systemId];
            if (registrySystem == null) {
                console.error(`System ${systemId} is not on the registry.`);
                utils.listAvailableSystems(registry);
...
function updateCore(downloadURL) {
    ...
    utils.downloadRelease(downloadURL, newCorePath, (err) => {
        if (err != null)
            utils.emitError("Failed to update the core.", err);
        for (const oldItem of ["client", "node_modules", "public", "server", "SupClient", "SupCore", "LICENSE.txt", "package.json", "README.md"])
            rimraf.sync(`${corePath}/${oldItem}`);
        for (const newItem of fs.readdirSync(newCorePath))
            fs.renameSync(`${newCorePath}/${newItem}`, `${corePath}/${newItem}`);
        rimraf.sync(newCorePath);
        console.log("Server successfully updated.");
        process.exit(0);
    });
}
function updateSystem(systemId, downloadURL) {
    ...
    utils.downloadRelease(downloadURL, newSystemPath, (err) => {
        if (err != null)
            utils.emitError("Failed to update the system.", err);
...
function updatePlugin(systemId, pluginFullName, downloadURL) {
    ...
    utils.downloadRelease(downloadURL, newPluginPath, (err) => {
        if (err != null)
            utils.emitError("Failed to update the plugin.", err);
