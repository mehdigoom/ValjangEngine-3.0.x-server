...
class TextEditorWidget {
    constructor(projectClient, clientId, textArea, options) {
        this.tmpCodeMirrorDoc = new CodeMirror.Doc("");
        this.texts = [];
        this.undoStack = [];
        this.undoQuantityByAction = [];
        this.redoStack = [];
        this.redoQuantityByAction = [];
        this.useSoftTab = true;
        this.linkElt = SupClient.html("link", { parent: document.head, rel: "stylesheet" });
        this.beforeChange = (instance, change) => {
            if (change.origin === "setValue" || change.origin === "network")
                return;
            const lastText = instance.getDoc().getValue();
            if (lastText !== this.texts[this.texts.length - 1])
                this.texts.push(lastText);
        };
        this.edit = (instance, changes) => {
            if (this.editCallback != null)
                this.editCallback(this.codeMirrorInstance.getDoc().getValue(), changes[0].origin);
            let undoRedo = false;
            let operationToSend;
            for (let changeIndex = 0; changeIndex < changes.length; changeIndex++) {
                ...
                for (let line = 0; line < change.from.line; line++)
                    operation.retain(this.tmpCodeMirrorDoc.getLine(line).length + 1);
            ...
            if (!undoRedo) {
                ...
                if (this.undoQuantityByAction[this.undoQuantityByAction.length - 1] > 20)
            ...
            if (this.sentOperation == null) {
                ...
                if (this.pendingOperation != null)
                    this.pendingOperation = this.pendingOperation.compose(operationToSend);
                else
                    this.pendingOperation = operationToSend;
            }
        };
    ...
    receiveEditText(operationData) {
        ...
        if (this.sentOperation != null) {
            ...
            if (this.pendingOperation != null)
                [this.pendingOperation, operation] = this.pendingOperation.transform(operation);
        }
                ...
                case "insert":
                    {
                        ...
                        for (let textIndex = 0; textIndex < texts.length; textIndex++) {
                            ...
                            if (textIndex !== texts.length - 1)
                                text += "\n";
                            this.codeMirrorInstance.replaceRange(text, { line, ch: cursorPosition }, null, origin);
                        ...
                        if (line === cursor.line && cursorPosition === cursor.ch) {
                            if (!operation.gotPriority(this.clientId)) {
                                for (let i = 0; i < op.attributes.text.length; i++)
                ...
                case "delete": {
                    ...
                    for (let textIndex = 0; textIndex < texts.length; textIndex++) {
                        ...
                        if (texts[textIndex + 1] != null)
                            this.codeMirrorInstance.replaceRange("", { line, ch: cursorPosition }, { line: line + 1, ch: 0 }, origin);
                        else
                            this.codeMirrorInstance.replaceRange("", { line, ch: cursorPosition }, { line, ch: cursorPosition + text.length }, origin);
                        if (moveCursor)
                            this.codeMirrorInstance.setCursor(line, cursorPosition);
                    }
                    break;
...
module.exports = TextEditorWidget;
