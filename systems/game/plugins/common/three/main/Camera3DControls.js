...
class Camera3DControls {
    constructor(root, camera, canvas) {
        ...
        this.orbitRadius = initialOrbitRadius;
        this.targetOrbitRadius = initialOrbitRadius;
        this.moveVector = new THREE.Vector3();
        this.pivotMarkerOpacity = 0;
        this.onMouseDown = (event) => {
            if (!this.enabled)
                return;
            if (this.wantToPan || this.isOrbiting)
                return;
            if (event.button === 2) {
                this.wantToPan = true;
                this.hasMovedWhilePanning = false;
        ...
        this.onWheel = (event) => {
            if (!this.enabled)
                return;
            if (event.deltaY > 0)
                this.targetOrbitRadius *= zoomingSpeed;
        ...
        pivotGeometry.vertices.push(new THREE.Vector3(-0.5, 0, 0), new THREE.Vector3(0.5, 0, 0), new THREE.Vector3(0, -0.5, 0), new THREE.Vector3(0, 0.5, 0), new THREE.Vector3(0, 0, -0.5), new THREE.Vector3(0, 0, 0.5));
    ...
    resetOrbitPivot(position, radius) {
        if (!this.enabled)
            return this;
        this.targetOrbitPivot.copy(position);
        if (radius != null)
            this.targetOrbitRadius = Math.min(Math.max(radius, minOrbitRadius), maxOrbitRadius);
        else
    ...
    update() {
        ...
        if (Math.abs(this.targetGamma - this.gamma) > epsilon)
            this.gamma += (this.targetGamma - this.gamma) * lerpFactor;
        else
            this.gamma = this.targetGamma;
